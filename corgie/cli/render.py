    ...
    def task_generator(self):
        for mip in self.mips:
            chunks = self.dst_stack.get_layers()[0].break_bcube_into_chunks(
                bcube=self.bcube, chunk_xy=self.chunk_xy, chunk_z=self.chunk_z, mip=mip
            )
...
class RenderJobWithGenerator(scheduling.Job):
    def __init__(
        self,
        src_stack,
        dst_stack,
...
class RenderTask(scheduling.Task):
    ...
    def execute(self):
        ...
        seethrough_bcube = self.bcube.translate(z_offset=self.seethrough_offset)
        ...
        for l in write_layers:
            ...
            if self.blackout_masks or self.seethrough:
                ...
                if (
                    l.get_layer_type() == "img"
                    and self.blackout_masks
...
@corgie_option("--lazy", is_flag=True)
@click.pass_context
def render(
    ctx,
    src_layer_spec,
    ...
    if not lazy:
        ...
        render_job = RenderJobWithGenerator(
            src_stack=src_stack,
            dst_stack=dst_stack,
            mips=mips,
            pad=pad,
